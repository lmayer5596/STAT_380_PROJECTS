...
train <- all_data[grep('^train', Id)][, .(Age, YearBuilt, LotArea, GrLivArea, CentralAir, TotalBsmtSF, Cond, Qual, SalePrice)]
#selects the rows which are labelled as test
test <- all_data[grep('^test', Id)][, .(Id, Age, YearBuilt, LotArea, GrLivArea, CentralAir, TotalBsmtSF, Qual, Cond, SalePrice)]
